...
async function getCopilotUsage(options) {
    try {
        ...
        const includedCredits = Math.max(1, Number(options.includedCredits || 1000));
        const autoModel = normalizeModel(options.autoModel || 'gpt-5.3-codex');
        const sinceMs = Date.now() - lookbackDays * 24 * 60 * 60 * 1000;
        const roots = getVsCodeWorkspaceStorageRoots().filter((root) => fs.existsSync(root));
        if (roots.length === 0) {
            return {
                fiveHour: null,
                sevenDay: null,
                error: formatUsageErrorDetail('copilot', 'No VS Code workspaceStorage directories found'),
            };
        }
        const transcriptFiles = collectWorkspaceFiles(roots, 'transcript').slice(0, COPILOT_MAX_FILES_PER_SOURCE);
        const chatFiles = collectWorkspaceFiles(roots, 'chat').slice(0, COPILOT_MAX_FILES_PER_SOURCE);
        const records = [];
        ...
        if (records.length === 0) {
            return {
                fiveHour: null,
                sevenDay: null,
                error: formatUsageErrorDetail('copilot', 'No Copilot usage records found'),
            };
        }
        const totals = {
            inputTokens: 0,
        ...
        for (const record of records) {
            ...
            totals.modelCredits.set(pricing.pricingModel, (totals.modelCredits.get(pricing.pricingModel) ?? 0) + pricing.credits);
        ...
        const usedPercent = Math.max(0, Math.min(100, Math.round((totals.credits / includedCredits) * 100)));
        const zeroOutputPct = Math.round((totals.zeroOutput * 100) / records.length);
        const tooltipNotes = [
        ...
        if (exactTranscriptSessions > 0) {
            tooltipNotes.push(`${exactTranscriptSessions} session(s) used exact transcript shutdown metrics`);
        }
        if (totals.unknownModels.size > 0) {
            tooltipNotes.push(`Unknown pricing model(s): ${Array.from(totals.unknownModels).slice(0, 3).join(', ')}`);
        }
        if (zeroOutputPct >= 50) {
            tooltipNotes.push('High zero-output ratio detected; historical Copilot logs may be incomplete.');
        }
        ...
        for (const model of topPricedModels) {
            ...
            tooltipNotes.push(`${model}: $${modelSpendUsd.toFixed(2)} (in $${formatRate(rate.input)}, cached $${formatRate(rate.cachedInput)}, out $${formatRate(rate.output)} /1M)`);
        }
        return {
            fiveHour: {
        ...
        return {
            fiveHour: null,
            sevenDay: null,
            error: formatUsageErrorDetail('copilot', String(error?.message ?? error)),
        };
    }
}
...
function collectWorkspaceFiles(roots, mode) {
    const files = [];
...
function parseTranscriptFile(filePath, sinceMs) {
    const records = [];
    let sessionId;
...
function parseChatSessionFile(filePath, sinceMs) {
    const records = [];
    const { session, requests } = collectSessionRequests(filePath);
    const sessionModel = getSessionSelectedModel(session);
    const sessionId = session.sessionId || path.basename(filePath).replace(/\.jsonl$/i, '');
    for (const request of requests) {
        ...
        const inputTokens = roughTokens(metadata.renderedUserMessage) +
...
function collectSessionRequests(filePath) {
...
function setPath(target, jsonPath, value) {
    let cursor = target;
...
function resolveRequestModel(request) {
    ...
    const detailsMatch = details.match(/(gpt-[a-z0-9.-]+|claude-[a-z0-9.-]+|gemini-[a-z0-9.-]+|grok-[a-z0-9.-]+)/);
    if (detailsMatch?.[1]) {
...
function modelFromRequest(request, sessionModel) {
...
function getSessionSelectedModel(session) {
...
function estimateCopilotCredits(record, autoModel) {
    const rawModel = normalizeModel(record.model);
    const pricingModel = rawModel === 'auto' ? normalizeModel(autoModel) : rawModel;
    const price = COPILOT_MODEL_PRICES[pricingModel];
    if (!price) {
        return { credits: 0, pricingKnown: false, pricingModel };
    }
    const nonCachedInput = Math.max(0, record.inputTokens - record.cacheReadTokens);
    const usd = (nonCachedInput * price.input +
...
function getVsCodeWorkspaceStorageRoots() {
    const roots = [];
    if (process.platform === 'darwin') {
        roots.push(path.join(HOME, 'Library', 'Application Support', 'Code', 'User', 'workspaceStorage'), path.join(HOME, 'Library', 'Application Support', 'Code - Insiders', 'User', 'workspaceStorage'));
    }
    else if (process.platform === 'win32') {
        const appData = process.env.APPDATA;
        if (appData) {
            roots.push(path.join(appData, 'Code', 'User', 'workspaceStorage'), path.join(appData, 'Code - Insiders', 'User', 'workspaceStorage'));
        }
    }
    roots.push(path.join(HOME, '.config', 'Code', 'User', 'workspaceStorage'), path.join(HOME, '.config', 'Code - Insiders', 'User', 'workspaceStorage'));
