...
function getCopilotConfig() {
  ...
  const lookbackDays = Math.max(
    1,
    Math.min(365, Math.round(Number(config.get('copilotLookbackDays', 30)))),
  );
  const includedCredits = Math.max(
    1,
    Number(config.get('copilotIncludedCredits', 1000)),
  );
  const autoModel = String(
    config.get('copilotAutoModel', 'gpt-5.3-codex') ?? 'gpt-5.3-codex',
  )
    .trim()
    .toLowerCase();
  return {
...
function getDisplayConfig() {
  ...
  const weeklyExhaustedDisplay = normalizeWeeklyExhaustedDisplay(
    config.get('weeklyExhaustedDisplay', 'percent'),
  );
  ...
  const criticalThreshold = Math.max(
    warningThreshold,
    clampPercent(config.get('criticalThreshold', 85)),
  );
